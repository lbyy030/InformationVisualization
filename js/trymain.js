...
var donut_svg = d3.select("#donut-chart-area").append("svg")
...
$("#date-slider").slider({
    range: true,
    max: parseTime("1800").getTime(),
    min: parseTime("2000").getTime(),
    step: 86400000, // One day
    values: [parseTime("1800").getTime(), parseTime("2000").getTime()],
...
d3.json("data/trydata.json", function (error, data) {
    console.log(data);
    formattedData = data;
...
function update(data) {
    ...
    filteredData.map(function (country) {
        return country.filter(function (variable) {
            variable[selectedVariable]["Very important"] = +variable[selectedVariable]["Very important"];
            variable[selectedVariable]["Rather important"] = +variable[selectedVariable]["Rather important"];
            variable[selectedVariable]["Not very important"] = +variable[selectedVariable]["Not very important"];
            variable[selectedVariable]["Not at all important"] = +variable[selectedVariable]["Not at all important"];
            variable[selectedVariable]["No answer"] = +variable[selectedVariable]["No answer"];
            variable[selectedVariable]["Don´t know"] = +variable[selectedVariable]["Don´t know"];
            variable[selectedVariable]["number"] = +variable[selectedVariable]["number"];
    ...
    line = d3.line()
        ...
        .y(function (d) { return y(d["Very important"]); });
    ...
    function mousemove() {
        ...
        focus.attr("transform", "translate(" + x(d.year) + "," + y(d["Very important"]) + ")");
        focus.select("text").text(function () { return d3.format(".2%")(d["Very important"].toFixed(2) / 100); });
    ...
    if (selectedVariable == "work") {
        newText = "Work Percentage";
    } else if (selectedVariable == "friends") {
        newText = "Friends Important Percentage";
    } else {
        newText = "Leisure Time Percentage";
    }
    yLabel.text(newText);
...
function change(donutTryData) {
    ...
    path.enter()
        ...
        .attr("fill", function (d) { return donut_color(d.data.degree) })
